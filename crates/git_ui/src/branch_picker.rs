...
impl BranchList {
    ...
    /// An embedded (minibuffer) branch list in select mode: confirming an entry
    /// invokes `on_select` with the chosen branch instead of checking it out.
    fn new_embedded_select(
        workspace: WeakEntity<Workspace>,
        repository: Option<Entity<Repository>>,
        width: Rems,
        on_select: SelectBranchCallback,
